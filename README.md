...
**The fork: this version keeps the original `autoresearch` loop of short runs, real code changes, and measurable evaluation, but expands it through a more explicit Jungian use of the transcendent function. Instead of treating self-improvement as hill-climbing alone, the fork asks the agent to hold thesis and antithesis in active tension, confront contradiction directly, and then produce a third concrete form in `train.py` that tries to preserve the strongest parts of both. The upstream project is intentionally minimalist and primarily organized around `val_bpb`, kept improvements, and a lean prompt surface. This fork adds that dialectical research psychology to `program.md`, runner-managed Codex `prepare` and `reflect` phases, remote SSH and Runpod execution, canonical `experiment_logs/` and `knowledge_base/` layers, and the `experiment-atlas` interface for live and historical exploration. The aim is still better models, but also a clearer record of what each experiment opposed, what contradiction it exposed, and what synthesis it was trying to bring into form.**
